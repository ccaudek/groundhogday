...
subj_codes <- rio::import(
  ...
  dplyr::select(
    subj_code_1, Task1Rev_n_1,
    context_1, control_1,
    post_context_1, coin, TIME_start,
    TIME_end, TIME_total
  )

subj_codes <- subj_codes[!is.na(subj_codes$Task1Rev_n_1), ]
subj_codes$subj_idx <- subj_codes$Task1Rev_n_1
...
dir <- here::here("data", "raw", "control_data")
  # snakemake@input[["dir_data"]]

# For the sessions with reversal:
file_names <-
  as.character(list.files(path = dir, pattern = "prl_EMA_rev"))
...
print("Saving output")
saveRDS(d, "data_processed/raw_data.Rds")
